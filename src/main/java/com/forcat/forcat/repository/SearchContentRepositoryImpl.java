...
import com.forcat.forcat.dto.ContentImageDTO;
import com.forcat.forcat.dto.ContentListAllDTO;
import com.forcat.forcat.dto.ContentListReplyCountDTO;
...
public class SearchContentRepositoryImpl extends QuerydslRepositorySupport implements SearchContentRepository {
    ...
    public Page<ContentListReplyCountDTO> searchWithContentReplyCount(String[] types, String keyword, Pageable pageable) {
        ...
        JPQLQuery<ContentListReplyCountDTO> dtoQuery = query.select(Projections.bean(ContentListReplyCountDTO.class,
                content.cno,
                content.ctitle,
                content.cwriter,
                content.regDate,
                contentReply.count().as("contentReplyCount")
        ));
    ...
    public Page<ContentListAllDTO> searchWithContentAll(String[] types, String keyword, Pageable pageable) {
        ...
        List<ContentListAllDTO> dtoCList = tupleList.stream().map(tuple -> {
            ...
            Content content1 = (Content) tuple.get(content);
            long replyCount = tuple.get(1,Long.class);

            ContentListAllDTO dto = ContentListAllDTO.builder()
                    .cno(content1.getCno())
                    .ctitle(content1.getCtitle())
                    .cwriter(content1.getCwriter())
                    .regDate(content1.getRegDate())
                    .creplyCount(replyCount)
                    .build();

            //BoardImage를 BoardImageDTO 처리할 부분
            List<ContentImageDTO> imageDTOS = content1.getImageSet().stream().sorted()
                    .map(contentImage -> ContentImageDTO.builder()
                            .uuid(contentImage.getUuid())
                            .fileName(contentImage.getFileName())
                            .ord(contentImage.getOrd())
                            .build()
