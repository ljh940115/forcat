...
public class ItemRepositoryCustomImpl implements ItemRepositoryCustom {
    ...
    private final JPAQueryFactory queryFactory;// JPAQueryFactory : Querydsl과 JPA를 이용하여 동적 쿼리 생성을 도와줌

    public ItemRepositoryCustomImpl (EntityManager em) { this.queryFactory = new JPAQueryFactory (em); }// 생성자를 통해 JPAQueryFactory에 EntityManager 객체를 넣어줌
    ...
    public Page<Item> getAdminItemPage (ItemSearchDto itemSearchDto, Pageable pageable) {
        QueryResults<Item> results = queryFactory.selectFrom (QItem.item).where (regDtsAfter (itemSearchDto.getSearchDateType ()), searchSellStatusEq (itemSearchDto.getSearchSellStatus ()), searchByLike (itemSearchDto.getSearchBy (), itemSearchDto.getSearchQuery ()))
                // where 조건절을 이용하여 BooleanExpression 반환 조건문 입력
                // ,를 이용하면 and 조건 인식
                .orderBy (QItem.item.id.desc ()).offset (pageable.getOffset ()) // 데이터를 가지고 올 시작 인덱스 지정
