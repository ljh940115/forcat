...
import com.forcat.forcat.dto.notice.NoticeImageDTO;
...
public class SearchNoticeRepositoryImpl extends QuerydslRepositorySupport implements SearchNoticeRepository {
    ...
    public SearchNoticeRepositoryImpl() { super(Notice.class); }

    @Override
    public Page<Notice> searchNotice1(Pageable pageable) {

        QNotice notice = QNotice.notice;//Q도메인 객체
        JPQLQuery<Notice> query = from(notice);//select...from board
        query.where(notice.noticeTitle.contains("1"));//where title like...

        //Querydsl로 Pageable 처리, paging
        this.getQuerydsl().applyPagination(pageable, query);

        List<Notice> list = query.fetch();//fetch() JPQL 실행
    ...
    @Override
    public Page<Notice> searchNoticeAll(String[] types, String keyword, Pageable pageable) {

        QNotice notice = QNotice.notice;
        JPQLQuery<Notice> query = from(notice);//QBoard를 이용하여 JPQLQuery 생성
            ...
            for (String type : types) {//types 배열을 순회하며 각 타입에 따라 검색 조건 추가
                ...
                switch (type) {//type 값에 따라 검색 조건 적용
                    case "t"://게시글 제목 검색 조건 추가
                        booleanBuilder.or(notice.noticeTitle.contains(keyword));
                        break;
                    case "c"://게시글 내용 검색 조건 추가
                        booleanBuilder.or(notice.noticeContent.contains(keyword));
                        break;
                    case "w"://게시글 작성자 검색 조건 추가
                        booleanBuilder.or(notice.noticeWriter.contains(keyword));
                        break;
                }
        ...
        //bno >0
        query.where(notice.noticeNo.gt(0L));//게시글 번호가 0보다 큰 경우 추가 검색

        //paging
        this.getQuerydsl().applyPagination(pageable, query);//페이지 네이션 적용하기 위해 pageable 사용, 쿼리에 페이지네이션 정보 추가
        List<Notice> list = query.fetch();//쿼리 실행하여 검색 결과 가져옴
    ...
    @Override
    public Page<NoticeListReplyCountDTO> searchWithNoticeReplyCount(String[] types, String keyword, Pageable pageable) {

        QNotice notice = QNotice.notice;
        QNoticeReply noticeReply = QNoticeReply.noticeReply;//QReply 객체 생성

        JPQLQuery<Notice> query = from(notice);//from(board)을 사용하여 게시판(Board) 테이블을 대상으로 하는 JPQLQuery를 생성
        query.leftJoin(noticeReply).on(noticeReply.notice.eq(notice));//게시글, 댓글 테이블 조인

        query.groupBy(notice);//결과를 게시글로 그룹화
        ...
        //bno > 0
        query.where(notice.noticeNo.gt(0L));//게시물 번호가 0보다 큰 경우 조건 추가

        //Brojections.bean 메서드 사용하여 BoardListReplyCountDTO 클래스 매핑할 프로젝션 설정
        JPQLQuery<NoticeListReplyCountDTO> dtoQuery = query.select(Projections.bean(NoticeListReplyCountDTO.class,
                notice.noticeNo,
                notice.noticeTitle,
                notice.noticeWriter,
                notice.regDate,
                noticeReply.count().as("noticeReplyCount")
        ));

        this.getQuerydsl().applyPagination(pageable, dtoQuery);//페이지네이션 적용
        List<NoticeListReplyCountDTO> dtoNoticeList = dtoQuery.fetch();//쿼리 결과 리스트 형태로 가져옴
        long count = dtoQuery.fetchCount();//전체 결과 수 가져옴

        return new PageImpl<>(dtoNoticeList, pageable, count);//결과 리스트, 페이지 정보, 전체 결과 수를 포함하는 페이지 객체 생성하고 반환
    }

    //다대일 문제 확인
    @Override//BoardListAllDTO 타입의 페이지(Page)를 반환하는 메서드, 게시물 목록과 댓글 수 정보가 포함
    public Page<NoticeListAllDTO> searchWithNoticeAll(String[] types, String keyword, Pageable pageable) {

        QNotice notice = QNotice.notice;
        QNoticeReply noticeReply = QNoticeReply.noticeReply;

        JPQLQuery<Notice> noticeJPQLQuery = from(notice);//게시물을 조회하기 위한 쿼리를 생성
        noticeJPQLQuery.leftJoin(noticeReply).on(noticeReply.notice.eq(notice)); //게시물과 댓글에 왼쪽 조인
        ...
        noticeJPQLQuery.groupBy(notice);// 게시물을 그룹화

        getQuerydsl().applyPagination(pageable, noticeJPQLQuery); //페이지 정보에 대한 쿼리를 생성

        //게시물과 댓글 수를 함께 조회, select는 게시물과 댓글 수를 함께 선택하도록 설정
        JPQLQuery<Tuple> tupleJPQLQuery = noticeJPQLQuery.select(notice, noticeReply.countDistinct());
        ...
        List<NoticeListAllDTO> dtoNoticeList = tupleList.stream().map(tuple -> {

            Notice notice1 = (Notice) tuple.get(notice);
            long replyCount = tuple.get(1,Long.class);

            NoticeListAllDTO dto = NoticeListAllDTO.builder()
                    .noticeNo(notice1.getNoticeNo())
                    .noticeTitle(notice1.getNoticeTitle())
                    .noticeWriter(notice1.getNoticeWriter())
                    .regDate(notice1.getRegDate())
                    .noticeReplyCount(replyCount)
                    .build();
        ...
        return new PageImpl<>(dtoNoticeList, pageable, totalCount);
    }
