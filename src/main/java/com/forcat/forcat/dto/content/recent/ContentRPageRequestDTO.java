...
@AllArgsConstructor
@NoArgsConstructor
public class ContentRPageRequestDTO {
    ...
    public Pageable getCRPageable (String... props) {
        ...
        return PageRequest.of (this.crpage - 1, this.crsize, Sort.by (props).descending ());
    }

    public String getCRLink () {
        if (crlink == null) {//link가 null인 경우에 실행
            StringBuilder builder = new StringBuilder ();//StringBuilder 객체 생성.page와 size 값을 URL 문자열에 추가
            builder.append ("cpage=" + this.crpage);
            builder.append ("&csize=" + this.crsize);
